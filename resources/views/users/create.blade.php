...
<div class="container mx-auto px-3 md:px-4">
  <div class="mb-4 flex items-center justify-between">
    <h1 class="text-xl font-semibold text-slate-100">Tambah User</h1>
  ...
  <div class="m-card p-5 max-w-3xl space-y-5">
    <form method="POST" action="{{ route('users.store') }}" class="space-y-5"
      ...
      <div class="grid md:grid-cols-2 gap-5">
        <div>
          ...
          <input
            ...
            value="{{ old('name') }}"
            required
            class="m-inp w-full rounded-xl px-3 py-2
        ...
        <div class="md:col-span-2 md:max-w-md">
          ...
          <input
            ...
            class="m-inp w-full rounded-xl px-3 py-2
                   ...
                   text-slate-100 placeholder-slate-500
                   focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            placeholder="Kosongkan = random 10 karakter">
          <p class="text-xs text-slate-400 mt-1">Jika dikosongkan, sistem akan membuat password acak.</p>
        </div>
      </div>
      ...
      <div>
        ...
        @if(empty($roles))
          <div class="rounded-xl bg-amber-900/40 border border-amber-700 px-4 py-3 text-amber-100">
            Belum ada role di database. Masuk ke <em>Kelola Role</em> untuk menambah role.
          </div>
        @else
