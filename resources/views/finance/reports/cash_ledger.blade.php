...
<div class="m-card p-5 mb-4">
  ...
  <form method="GET" class="grid md:grid-cols-6 gap-3 mb-4">
    ...
    <div class="md:col-span-2 flex gap-2">
      ...
      @if(Route::has('finance.cash.export.csv') && $account)
        <a class="m-btnp" 
	  href="{{ route('finance.cash.export.csv', ['account_id'=> $account->id, 'start'=>$start, 'end'=>$end]) }}">Export CSV
	</a>
      @endif

      @if(Route::has('finance.cash.export.pdf'))
        <a class="m-btnp" href="{{ route('finance.cash.export.pdf', request()->query()) }}">Export PDF</a>
      @endif
    </div>
    ...
    <table class="w-full text-sm">
      <thead>
        <tr>
          ...
          <th class="text-right p-2">Masuk</th>
          <th class="text-right p-2">Keluar</th>
      ...
      <tbody>
        ...
        @foreach($lines as $l)
          @php
            $masuk = (float) $l->debit;
            $keluar= (float) $l->credit;
            $running += $masuk - $keluar; // karena kas adalah Asset (saldo normal debit)
          @endphp
          <tr class="border-t border-slate-700">
            <td class="p-2">{{ $l->entry->date->format('Y-m-d') }}</td>
            <td class="p-2">{{ $l->entry->ref }}</td>
            <td class="p-2">{{ $l->entry->description }}</td>
            <td class="p-2 text-right">{{ number_format($masuk,2) }}</td>
            <td class="p-2 text-right">{{ number_format($keluar,2) }}</td>
