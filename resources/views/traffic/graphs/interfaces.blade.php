...
<div class="card p-4 mb-4">
  <div class="label mb-2">Tambah Target Interface</div>
  <form method="POST" action="{{ route('traffic.targets.store') }}" class="grid md:grid-cols-12 gap-2">
    ...
    <div class="md:col-span-5">
      <input name="target_key" class="field" placeholder="ether1" required>
    </div>
    <div class="md:col-span-3">
...
<div class="card p-4">
  <div class="label mb-2">Daftar Interface</div>
  <table class="w-full text-sm">
    <thead>
      <tr class="text-left text-[var(--muted)]">
        <th class="py-2">Label / Interface</th>
          ...
          <td class="p-2 text-right">
            <div class="flex items-center justify-end gap-2">
              ...
              <form method="POST" action="{{ route('traffic.targets.destroy',$r->id) }}" class="inline-block"
                    onsubmit="return confirm('Hapus target Interface ini?');">
                @csrf
                @method('DELETE')
