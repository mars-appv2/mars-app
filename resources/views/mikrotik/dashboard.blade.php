...
<div class="container mx-auto px-2 md:px-4">
  ...
  <div class="m-card">
    ...
    <button id="btnStop"  class="m-btn">Stop</button>
     ...
     <!-- === /Group Toolbar (v3) === -->
      <div id="groupBar" class="flex flex-wrap items-center gap-2 mt-3">
        <select id="gSel" class="m-sel" style="min-width:220px"><option value="">— pilih group —</option></select>
        <input id="gName" class="m-inp" placeholder="Nama group (mis: uplink)" style="min-width:240px"/>
        <button id="gSave" class="m-btn">Simpan</button>
        <button id="gDel"  class="m-btn">Hapus</button>
        <button id="gEdit" class="m-btn">Edit</button>
        <button id="gRun"  class="m-btn m-btnp">Mulai</button>
      </div>
    </div>
  </div>
</div>
...
<script>/* group-local-v3 */(function(){
  const MK_ID = (function(){try{return {{ $mikrotik->id ?? 0 }}}catch(e){return 0}})();
  const KEY   = `mk_groups_${MK_ID}`;
  const q   = (s,root=document)=>root.querySelector(s);
  const qa  = (s,root=document)=>Array.from(root.querySelectorAll(s));
  const uniq = arr => Array.from(new Set(arr));
  const getStore = ()=>{ try{ return JSON.parse(localStorage.getItem(KEY)||"{\"groups\":{}}")}catch(e){ return {"groups":{}} } };
  const setStore = (obj)=>{ localStorage.setItem(KEY, JSON.stringify(obj)); };
  function getChipsIfaces(){
    const spans = qa("#chips span");
    const list = [];
    if (spans.length){
      spans.forEach(sp=>{
        let txt = sp.cloneNode(true); 
        qa("button",txt).forEach(b=>b.remove());
        let name = txt.textContent.replace(/×/g,"").trim();
  ...
  function renderGroupOptions(){
    const sel = q("#gSel"); if(!sel) return;
    const gs = getStore().groups || {}; 
    const cur = sel.value; sel.innerHTML = "<option value=\"\">— pilih group —</option>";
    Object.keys(gs).sort((a,b)=>a.localeCompare(b)).forEach(n=>{
      const opt = document.createElement("option"); 
      opt.value = n; opt.textContent = `${n} (${(gs[n]||[]).length})`; 
      sel.appendChild(opt); 
    });
    if (cur && gs[cur]) sel.value = cur;
  }
  function clearChips(){ qa("#chips span button").forEach(btn=>btn.click()); }
  function clickAdd(iface){ const sel=q("#iface"); const btn=q("#btnAdd"); if(sel && btn){ sel.value=iface; btn.click(); } }
  function clickStart(){ q("#btnStart")?.click(); }
  function saveGroup(){
    const name = (q("#gName")?.value||"").trim();
    const ifs  = getChipsIfaces();
    if(!name){ alert("Isi nama group"); return; }
    if(!ifs.length){ alert("Tambah minimal 1 interface"); return; }
    const st = getStore(); st.groups = st.groups || {}; st.groups[name] = uniq(ifs); setStore(st);
    renderGroupOptions();
    q("#gSel").value = name; 
    alert("Group tersimpan");
  }
  function deleteGroup(){
    const sel=q("#gSel"); const v=sel?.value||""; if(!v){ alert("Pilih group dulu"); return; }
    if(!confirm(`Hapus group \"${v}\" ?`)) return;
    const st=getStore(); if(st.groups && st.groups[v]){ delete st.groups[v]; setStore(st); }
    renderGroupOptions();
    alert("Group dihapus");
  }
  async function editGroup(){
    const v = (q("#gSel")?.value)||""; if(!v){ alert("Pilih group dulu"); return; }
    const st = getStore(); const list = st.groups?.[v] || [];
    clearChips();
    list.forEach(x=>clickAdd(x));
    q("#gName").value = v; q("#gName").focus();
  }
  async function runGroup(){
    const v = (q("#gSel")?.value)||""; if(!v){ alert("Pilih group dulu"); return; }
    const st = getStore(); const list = uniq(st.groups?.[v] || []);
    if(!list.length){ alert("Group kosong"); return; }
    clearChips();
    list.forEach(x=>clickAdd(x));
    clickStart();
  }
  document.addEventListener("DOMContentLoaded",()=>{
    q("#gSave")?.addEventListener("click", saveGroup);
    q("#gDel") ?.addEventListener("click", deleteGroup);
    q("#gEdit")?.addEventListener("click", editGroup);
    q("#gRun") ?.addEventListener("click", runGroup);
    renderGroupOptions();
  });
})();</script>
@endsection
