...
class MikrotikController extends Controller
{
    ...
    /**
     * Route mikrotik.pppoe.add:
     *  - mode=client (default): tambah PPP secret
     *  - mode=profile        : tambah PPP profile
     */
    public function pppAdd(Mikrotik $mikrotik, Request $r){
        ...
        try{
            ...
            if ($mode === 'profile') {
                ...
                $svc->pppAdd($d['name'],$d['password'],$d['profile']?:'default');
                if($r->has('record')){
    ...
    /** Route mikrotik.pppoe.edit (ubah profil/password, enable/disable, toggle rekam) */
    ...
    public function pppRecord(\App\Models\Mikrotik $mikrotik, \Illuminate\Http\Request $r){
        ...
        $mt = \App\Models\MonitorTarget::where([
            "mikrotik_id"=>$mikrotik->id, "target_type"=>"pppoe", "target_key"=>$name
        ])->first();
        if ($on) {
            if (!$mt) {
                \App\Models\MonitorTarget::create([
                    "mikrotik_id"=>$mikrotik->id, "target_type"=>"pppoe", "target_key"=>$name,
                    "label"=>$label, "enabled"=>true
                ]);
            } else { $mt->update(["enabled"=>true, "label"=>$label]); }
